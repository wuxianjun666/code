...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features #64
        hidden_features = hidden_features or in_features#256
...
class DynamicPosBias(nn.Module):
    def __init__(self, dim, num_heads, residual):
        ...
        self.pos_dim = dim // 4 #16
        self.pos_proj = nn.Linear(2, self.pos_dim) #2->16
        self.pos1 = nn.Sequential(
            nn.LayerNorm(self.pos_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.pos_dim, self.pos_dim),#16->16
        )
        ...
        self.pos3 = nn.Sequential(
            nn.LayerNorm(self.pos_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.pos_dim, self.num_heads) #16->2
        )
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, group_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        super().__init__()
        self.dim = dim#64
        self.group_size = group_size  # Wh, Ww  #(7,7)
        self.num_heads = num_heads    #2
        ...
        if position_bias:
            self.pos = DynamicPosBias(self.dim // 4, self.num_heads, residual=False) #16 #2
            
            # generate mother-set
            position_bias_h = torch.arange(1 - self.group_size[0], self.group_size[0])
            position_bias_w = torch.arange(1 - self.group_size[1], self.group_size[1])
            biases = torch.stack(torch.meshgrid([position_bias_h, position_bias_w]))  # 2, 2Wh-1, 2W2-1 #(y,x)
            biases = biases.flatten(1).transpose(0, 1).float() #(x,y)
            ...
            relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 2, Wh*Ww, Wh*Ww #（2，49，49）
            relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # Wh*Ww, Wh*Ww, 2 #（49，49，2）
        ...
        self.proj = nn.Linear(dim, dim) #（64，64）
...
class CrossFormerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, group_size=7, lsda_flag=0,
        ...
        self.num_heads = num_heads#2
        self.group_size = group_size #7
        self.lsda_flag = lsda_flag
        self.mlp_ratio = mlp_ratio#4
        self.num_patch_size = num_patch_size#4
        ...
        self.attn = Attention(
            dim, group_size=to_2tuple(self.group_size), num_heads=num_heads,#64 #[7,7] #2
...
class Stage(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, group_size,
        ...
        for i in range(depth):
            ...
            self.blocks.append(CrossFormerBlock(dim=dim, input_resolution=input_resolution,#64 #[56,56]
                                 num_heads=num_heads, group_size=group_size, #2 #7
                                 lsda_flag=lsda_flag,
                                 ...
                                 num_patch_size=num_patch_size)) #4
...
if __name__ == '__main__':
    ...
    '''
    model = CrossFormer(img_size=[1280, 960],
                        patch_size=[4, 8, 16, 32],
                        in_chans=3,
                        embed_dim=128,
                        depths=[2, 2, 18, 2],
                        ...
                        merge_size=[[2, 4], [2, 4], [2, 4]])
    input = torch.randn(1, 3, 1280, 960)
    output = model(input)
    print(output)
