...
class SegDecNet(nn.Module):
    def __init__(self, device, input_width, input_height, input_channels):
        ...
        # state_dict = torch.load("/devdata/cord/crossformer-l.pth")['model']
        # new_swin_state_dict = self.volume.state_dict()
        # for x in new_swin_state_dict:
        #     if x.endswith("relative_position_index") or x.endswith("attn_mask"):
        #         pass
        #     else:
        #         new_swin_state_dict[x] = state_dict[x]
        # self.volume.load_state_dict(new_swin_state_dict)
    ...
    def forward(self, input):
        ...
        volume = self.volume.patch_embed(input)
        volume = self.volume.pos_drop(volume)
        for layer in self.volume.layers:
            volume = layer(volume)
        B, L, C = volume.shape
